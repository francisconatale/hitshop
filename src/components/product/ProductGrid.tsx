...
export default function ProductGrid({ category }: { category?: string }) {
  ...
  if (category) {
    ...
    return (
      <div className="max-w-[1440px] mx-auto my-6 px-4">
        <div className="border border-on-surface bg-surface flex items-center justify-between p-4 mb-0">
          <div className="flex items-center gap-4">
             ...
             <div>
                <div className="text-[8px] font-mono font-black text-primary-dim/60 uppercase tracking-[0.2em] leading-none mb-2 px-2 py-1 bg-primary/5 inline-block border-l-2 border-primary/20">
                  SYSTEM / HARDWARE
                </div>
                <h1 className="text-4xl font-black uppercase tracking-tighter leading-none text-on-surface">
        ...
        <div className="border-l border-t border-on-surface">
          {categoryProducts.length > 0 ? (
            <section className={`grid grid-cols-2 md:grid-cols-3 ${isPeripherals ? 'lg:grid-cols-5' : 'lg:grid-cols-4'}`}>
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 py-12 overflow-hidden">
      {/* Title Sequence */}
      <div className="mb-32 relative">
        <div className="absolute -top-12 left-0 text-[8px] font-mono font-black text-primary-dim/70 tracking-[0.3em] uppercase flex items-center gap-3 px-3 py-1.5 bg-primary/5 border-l-2 border-primary/30">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 border-b-[24px] border-on-surface pb-12 overflow-hidden relative">
          {/* Neon Trace Line */}
          <div className="absolute bottom-[24px] left-0 w-full h-[2px] bg-primary-fixed/30 z-10" />

          <div className="max-w-5xl relative p-4">
            {/* Subtle Corner Brackets */}
            <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-primary-fixed/20" />
            <div className="absolute top-0 right-0 w-2 h-8 border-t-2 border-r-2 border-primary-fixed/20" />

            <h1 className="text-[8rem] md:text-[12rem] font-black uppercase tracking-tighter leading-[0.75] text-on-surface relative flex flex-wrap">
              ...
              <span className="absolute -right-8 top-0 text-[10px] font-mono text-primary-fixed/40 [writing-mode:vertical-lr]">
                PRT_77-X
              </span>
            </h1>
          </div>
          ...
          <AnimatePresence>
            {isHeroComplete && (
              <motion.div 
                ...
                className="flex flex-col items-start lg:items-end gap-6 lg:mb-4"
              >
                <span className="text-2xl font-black bg-on-surface text-surface px-6 py-2 uppercase rotate-1 brutal-shadow-primary">
                  {texts.dropLabel}
                </span>
                <p className="max-w-md font-bold text-xl leading-[1.1] text-left lg:text-right uppercase text-on-surface-variant">
      ...
      >
        {['Status: Online', 'Uptime: 99.9%', 'Node: DC-04', 'Auth: Root'].map((text, i) => (
          <div key={text} className="border-r-2 border-b-2 border-on-surface p-6 font-mono text-[10px] font-black uppercase text-on-surface-variant opacity-50 hover:opacity-100 transition-opacity">
            {text}
          </div>
        ))}
      </motion.div>
