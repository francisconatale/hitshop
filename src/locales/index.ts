export const navItems = [
  { label: "HOME", href: "/", icon: "home" },
  { label: "GPUs", href: "/gpus", icon: "memory" },
  { label: "CPUs", href: "/cpus", icon: "developer_board" },
  { label: "SYSTEMS", href: "/systems", icon: "desktop_windows" },
  { label: "PERIPHERALS", href: "/peripherals", icon: "keyboard" },
];

export const sidebar = {
  title: "HITSHOP",
  subtitle: "RAW SILICON POWER",
};

export const productTexts = {
  heroTitle: "SYSTEM HARDWARE",
  heroDescription: "RAW PERFORMANCE. SILICON, POWER, AND THERMALS PUSHED TO THEIR LIMITS. NO COMPROMISES.",
  dropLabel: "Drop 01 / Vol 01",
  peripheralsTitle: "PERIPHERALS",
  peripheralsSubtitle: "curated selection",
  peripheralsDescription: "KEYBOARDS, MICE, MONITORS & ACCESSORIES",
  footerBigText: ["RAW", "SILICON", "POWER"] as const,
  copyright: "2026 SYSTEM HARDWARE",
  footerLinks: {
    terms: "Terms",
    shipping: "Shipping",
    instagram: "Instagram",
  },
  addButton: "ADD",
};
